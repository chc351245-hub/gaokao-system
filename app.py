...
================================================================================
 云端架构：
   1. 隐蔽式深度测评 — 宏观意愿题(10道) + 微观行为场景题(30道)
   2. 三层递进漏斗 — 门类初筛 → 专业类 Top 8 → 专业微观狙击(≤6/类)
================================================================================
 运行方式：
...
def compute_and_show_results() -> None:
    ...
    if not funnel_results:
        st.error("⚠️ 推荐引擎未返回结果。请检查数据文件是否完整：`label_checkpoints/` 目录下应有 4 个 JSON 文件，以及 `gaokao_majors.xlsx`。")
