...
# 说明：这里原先导入了 recommend_for_user / print_results / clamp，后来又只剩
# load_enhanced_majors。随着 load_majors() 被删除（见下方「第2部分」的说明），
# 最后一个引用也没了 —— recommendation_engine（v5.0，1209 行）与 app.py 彻底解耦。
# 该模块文件仍保留在仓库里备查，但已不参与任何运行时路径。
...
# 第2部分：专业数据加载
# ========================================================================
# 这里原本有一个 load_majors()，读 enhanced_majors.json（旧版 33 专业样本库）。
# 它唯一的调用点在 render_sidebar() 里，且返回值从未被使用——纯粹是死代码，
# 却让「每次页面加载都强制依赖一个 43KB 的遗留文件」：
#   · github_upload/.gitignore 已把 enhanced_majors.json 归入「旧数据（仅33专业）」不上传；
#   · load_enhanced_majors() 内部是裸 open()，没有 try/except。
# 两者相遇 → 按 github_upload/ 上传后，侧边栏一渲染就 FileNotFoundError，整站起不来。
# v6.0 引擎的数据全部来自 label_checkpoints/（883 专业），不需要这个文件，故整条链删除。
# 连带 effect：app.py 不再需要 import recommendation_engine（v5.0 遗留模块）。
...
def render_funnel_results(funnel_results: list[dict], user: UserProfile) -> None:
    ...
    for ci, cat in enumerate(funnel_results):
        ...
        with st.expander(
            ...
            # 展示改动说明：原来这里只显示一个「方向匹配分」，而那个数其实是
            #   industry_match*0.5 + asset_match*0.2 + score_match*0.3
            # ——把「你适不适合」和「你能不能上 / 家庭扛不扛得住」混在了一起，却又在
            # 页面底部写着「匹配分衡量你适不适合，不等同于你能不能考上」，自相矛盾。
            # 用户看到临床医学类 0.966 时无从判断：是契合度不够，还是完美契合但位次
            ...
            st.markdown(
                ...
                <div style="display:flex;gap:18px;align-items:flex-start;
                  ...
                  <div>
                    ...
                    <div style="font-size:20px;font-weight:600;color:{tier_color};">{tier}</div>
                    <div style="color:#888;margin-top:2px;">{tier_hint}</div>
                  </div>
                </div>
                """,
                unsafe_allow_html=True,
    ...
    st.markdown("---")
    st.info(
        "💡 **温馨提示**：每张卡片上的两个数字说的是两件事——\n\n"
        "- **契合度**：只衡量「你这个人」和「这个方向」的内在适配度，与分数无关。\n"
        "- **现实折损**：这个方向的分数位次门槛、家庭资源门槛，对照你当前条件的结果。\n\n"
        "**契合度高但现实折损也高**，说明你适合它、但需要位次或资源再加把劲（冲刺区）；"
        "**契合度高且现实折损低**，才是性价比最高的选择。"
        "实际填报请结合 **高考分数**、**院校偏好** 和 **城市选择** 综合决策。"
    )
    st.caption(
