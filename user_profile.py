...
# 产业方向定义（33 个细粒度方向）
# ============================================================================
# 历史上这里是 10 个「产业集群」，而专业类的产业标签有 33 个，两者之间靠
# `INDUSTRY_TAG_TO_CLUSTER` 做多对一压缩。那次压缩是两处长期缺陷的根源：
#   · 「政府公共」吞并了 法律服务/合规、科研/学术、军事/国防工业、房地产/物业，
#     一个维度覆盖 50/93 = 53.8% 的专业类，几乎不携带区分信息；
#   · 「智能制造」吞并了 物流/供应链、建筑/土木/城规、航空航天、海洋工程/船舶，
#     使「物流管理与工程类」凭空继承了两个最热门集群，实测 300 份随机答卷里
#     16 份被推为第一——而它真实的去向（物流/供应链）从未参与打分。
# 现在用户向量与专业类标签用**同一套 33 维词表**，压缩层被彻底删除。
# 见 funnel_engine.layer2_category_match 与 questionnaire.MACRO_QUESTIONS。
INDUSTRY_DIMENSIONS = [
    "人工智能/大模型",
    "互联网/软件",
    "半导体/集成电路",
...
# 兼容别名：recommendation_engine.py（已废弃的 v5.0 引擎）及其测试仍在 import 这个名字。
# 新代码请用 INDUSTRY_DIMENSIONS。
INDUSTRY_CLUSTERS = INDUSTRY_DIMENSIONS
